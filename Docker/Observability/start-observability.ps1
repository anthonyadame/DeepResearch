...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "    → NOTE: Changed from port 3000 to avoid conflict with open-webui" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host "  docker-compose -f docker-compose-monitoring.yml down" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host "  • Ports already in use (3001, 9090, 16686)" -ForegroundColor White
    Write-Host "  • NOTE: Port 3000 avoided due to open-webui conflict" -ForegroundColor White
    Write-Host ""
    Write-Host "To view logs:" -ForegroundColor Yellow
    Write-Host "  docker-compose -f docker-compose-monitoring.yml logs" -ForegroundColor DarkGray
}
